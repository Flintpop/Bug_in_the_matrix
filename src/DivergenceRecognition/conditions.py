...
class MacdDivergenceConditions:
    ...
    def divergence_spotter(self):
        ...
        if self.coin.long and word == 'long' or not self.coin.long and word == 'short':
            ...
            divergence = Core.comparator_numbers(self.coin.long, local[local_length - 1], local[local_length]) \
                         and Core.comparator_numbers(self.coin.long, macd[macd_length], macd[macd_length - 1])
            if divergence:
                if self.coin.long:
